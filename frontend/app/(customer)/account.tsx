...
// Collage dimensions
const C = 68;           // total container size
const G = 1;            // gap between tiles
const H = (C - G) / 2; // half = 33.5
...
type CheckoutGroup = {
  ...
  created_at: string;
  total: number;
  lineItemCount: number;       // distinct order_item rows → drives +N count
  images: (string | null)[];  // up to 4 for the collage
...
function PhotoCollage({ images, lineItemCount }: {
  images: (string | null)[];
...
export default function Account() {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('orders')
      .select('id, checkout_id, status, total_inr, created_at, store_name, order_items(quantity, pickle_image_url)')
          ...
          renderItem={({ item: group }) => {
            ...
            return (
              ...
              >
                ...
                {/* Row 2: collage · store name + price (inline, no extra row) */}
